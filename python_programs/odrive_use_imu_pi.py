...
class OdriveMotorControl(Node):
    def __init__(self):
        super().__init__('odrive_use_imu_pi')
        ...
        self.imu_yaw = 0.0
        self.pre_imu_yaw = 0.0
        self.diff_imu_yaw = 0.0
    ...
    def calcodom(self, current_time):
        if self.imu_flag:
            ...
            # ヨー角の差分を計算
            odrive_yaw = self.imu_yaw
            self.diff_imu_yaw = odrive_yaw - self.pre_imu_yaw
            th = self.diff_imu_yaw
            self.pre_imu_yaw = odrive_yaw
    ...
    def imu_callback(self, msg):
        ...
        self.imu_yaw = yaw  % (2*math.pi)

        # 初回のみ初期化
        if not self.imu_flag:
            self.init_imu_yaw = self.imu_yaw
            self.pre_imu_yaw = self.imu_yaw
        self.imu_yaw = self.imu_yaw - self.init_imu_yaw + self.init_yaw

        # ヨー角の差分を計算
        #self.diff_imu_yaw = self.imu_yaw - self.pre_imu_yaw
        #self.pre_imu_yaw = self.imu_yaw
        self.imu_flag = True

        # デバッグ用にログを出力
        self.get_logger().info(f'IMU Yaw: {self.imu_yaw}, Yaw Diff: {self.diff_imu_yaw}')
